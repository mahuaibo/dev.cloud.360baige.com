...
// @router /add [get]
func (c *ApplicationTplController) Add() {
	timestamp := time.Now().UnixNano() / 1e6
	var (
		res   Response // http 返回体
		reply ApplicationTpl
	)
	args := &ApplicationTpl{
		CreateTime: timestamp,
		UpdateTime: timestamp,
	}
	err := client.Call(beego.AppConfig.String("EtcdURL"), "ApplicationTpl", "Add", args, &reply)
	if err != nil {
...
func (c *ApplicationTplController) Detail() {
	...
	err := client.Call(beego.AppConfig.String("EtcdURL"), "ApplicationTpl", "FindById", args, &reply)
...
// @Failure 400 {"code":400,"message":"..."}
// @router /modify [post]
func (c *ApplicationTplController) Modify() {
	id, _ := c.GetInt64("id")
