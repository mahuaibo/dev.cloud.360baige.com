...
func (c *UserController) Login() {
	...
	loginType, _ := c.GetInt("loginType")// 1:账号密码登陆  2:微信登陆  3:qq登陆
	username := c.GetString("username")
	password := c.GetString("password")
	code := c.GetString("code")
	err := utils.Unable(map[string]string{"loginType": "int:true", "username": "string:false", "password": "string:false", "code":"string:false"}, c.Ctx.Input)
	if err != nil {
	...
	if loginType == 1 {
		...
		fmt.Println("userInfo", userInfo.Openid)
		err = client.Call(beego.AppConfig.String("EtcdURL"), "User", "FindByCond", action.FindByCond{
			CondList: []action.CondValue{
